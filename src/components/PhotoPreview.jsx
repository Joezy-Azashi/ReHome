...
function PhotoPreview({ data, preview, setPreview }) {
    const { t } = useTranslation();

    const [item, setItem] = useState(0)

    return (
        <Dialog open={preview} onClose={() => { setPreview(false) }} fullWidth maxWidth='lg'>
            <DialogContent sx={{ padding: '0 !important' }}>
                <TabContext value={"1"}>
                    <Box sx={{ borderBottom: 1, borderColor: 'lightgrey', bgcolor: 'secondary.main', color: '#fff', padding: '1rem', display: 'flex', justifyContent: "space-between" }}>
                        ...
                        <IconButton onClick={() => { setPreview(false); setItem(0) }}><Close color='paper' fontSize='small' /></IconButton>
                    </Box>
                    {/* Gallery */}
                                            ...
                                            data?.map((img, index) => {
                                                return (
                                                    <ImageListItem key={index} onClick={() => setItem(index)} sx={{ cursor: "pointer" }}>
                                                        <img src={`${img}?fit=crop&auto=format`} style={{ borderRadius: '6px', height: "10rem" }} alt={`property_img_${index}`} loading='lazy' />
                                                    </ImageListItem>
                                                )
                                            })
                                        }
