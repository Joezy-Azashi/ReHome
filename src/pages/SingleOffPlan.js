import { PrivacyTipOutlined, FireplaceOutlined, QrCodeOutlined, FeaturedPlayListOutlined, BedOutlined, ShowerOutlined, AccountBalanceWalletOutlined, FmdGoodOutlined, ReplyOutlined, MeetingRoom, Close, FiberManualRecord, Favorite, FavoriteBorder, Place, TaskAlt } from '@mui/icons-material'
import { TabContext } from '@mui/lab'
import { ExpandMore } from '@mui/icons-material'
import { alpha, Backdrop, Box, Avatar, Container, Accordion, Button, Chip, AccordionDetails, AccordionSummary, Divider, CircularProgress, Grid, IconButton, List, ListItem, ListItemIcon, ListItemText, Popover, styled, Typography, Tooltip, Hidden } from '@mui/material'
    ...
    const getSingleDevelopment = (id) => {
        ...
        Api().get(`/rehome-properties/${id[2]}`, {
            ...
            .then((response) => {
                ...
                setLoading(false)
                setHideQR(true)
                if(getCurrentUser()?.id === response?.data?.userId){
                    setHideQR(false)
                }
            })
            .catch((error) => {
                    ...
                    <Container maxWidth='xl'>
                        <Box sx={{ display: { sm: "flex", xs: "block" }, justifyContent: "space-between", alignItems: "center", padding: ".6rem 0", borderWidth: "1px 0" }}>
                            ...
                            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", marginTop: { xs: "1rem" } }}>
                                ...
                                <StyledButton onClick={(e) => { toggleWishlist(id[2]) }}>
                                    {load ? <CircularProgress size='1.33rem' color='primary' />
                                        : favs?.includes(id[2]) ? <Favorite color='secondary' fontSize='small' /> : <FavoriteBorder color='#757575' fontSize='small' />
                                    }
                                </StyledButton>
                            </Box>
                        </Box>
                        ...
                        <Grid container spacing={1}>
                            ...
                            <Hidden lgDown>
                                <Grid item xs={12} md={4} sx={{ display: "flex", justifyContent: "center" }}>
                                    <PropertyContactForm user={user} data={data} />
                                </Grid>
                            </Hidden>
                        </Grid>
                        ...
                        <Box py={5}>
                            <Box sx={{ display: "flex", alignItems: "center", gap: "10px" }}>
                                ...
                                <Box sx={{ display: "flex", alignItems: "center" }}>
                                    <Box>
                                        <Typography noWrap fontSize={"1.2rem"} fontWeight={"600"} width={{ xs: "90%", sm: "100%" }}>{data?.user?.agencies[0]?.name}</Typography>
                                        <FlexBox>
                        ...
                        <TabContext>
                            <Box sx={{ bgcolor: 'secondary.main', height: '4rem', padding: "0 1rem", borderColor: 'divider', color: '#fff', position: "sticky", top: "0", zIndex: '1', display: { xs: "none", sm: "flex" }, alignItems: "center", width: "100%", overflowX: "auto" }}>
                                <Box sx={{ display: "flex", gap: "22px" }}>
                                    ...
                                    {/* <Link activeClass="activenav" style={{ padding: "8px", cursor: "pointer" }} spy to="description">
                                        <Box sx={{ display: "flex", alignItems: "center", gap: "10px" }}>
                                            <DescriptionOutlined size="small" />
                                            {t("singleproperty.tab3.descriptiontitle")}
                                        </Box>
                                    </Link> */}
                                    <Link activeClass="activenav" style={{ padding: "8px", cursor: "pointer" }} spy to="units">
                                        <Box sx={{ display: "flex", alignItems: "center", gap: "10px" }}>
                                            <MeetingRoom size="small" />
                                            <span style={{ whiteSpace: "nowrap" }}>{t('offplan.singleoffplan.mainheader.tab2')}</span>
                                        </Box>
                                    </Link>
                                    ...
                                    <Grid container spacing={2}>
                                        <Grid item xs={12} md={10}>
                                            <Grid container spacing={2}>
                                                ...
                                                <Grid item xs={6} sm={9} padding={'1rem'}><Typography>{data?.name}</Typography></Grid>
                                                ...
                                                <Grid item xs={6} sm={8} md={9} marginTop={'1rem'}><Typography>{data?.description}</Typography></Grid>
                                            </Grid>
                                        </Grid>

                                        <Grid item xs={12} md={2}>
                                            <Box sx={{ display: 'flex', alignItems: "center", flexDirection: { xs: 'row', sm: 'row', md: 'column', lg: 'column' }, gap: '1rem', mb: { xs: '1rem', sm: '1rem', md: 0, lg: 0 } }}>
                                                {data?.documents?.find((hp) => hp?.document_type === "house_plan") ?
                                                    <RoundButton
                                                        onClick={() => window.open(data?.documents?.find((hp) => hp?.document_type === "house_plan")?.uri, '_blank')}
                                                        size={'small'}
                                                        text={t("singleproperty.viewplan")}
                                                        variant='contained'
                                                        disableElevation
                                                        color={'primary'}
                                                        sx={{ lineHeight: 1.3, borderRadius: { xs: '8px', sm: '8px' }, width: "100%" }}
                                                    /> : null}
                                                {/* <PopupButton
                                                ...
                                                <RoundButton
                                                    ...
                                                    size={'small'}
                                                    text={t("singleproperty.tab3.buttons.verify")}
                                                    variant='contained'
                                                    disableElevation
                                                    color={'primary'}
                                                    sx={{ lineHeight: 1.3, borderRadius: { xs: '8px', sm: '8px' }, width: "100%" }}
                                                />
                                                { !!qrImage && !hideQR &&<Box>
                                                    <Grid item xs={12} sm={12} md={12} lg={12} sx={{ width: '100%', height: "30vh", overflow: "visible" }}>
                            ...
                            <div id="units">
                                <Box sx={{ paddingTop: { xs: '50px', sm: '100px' } }}>
                                    <Typography variant='h5' mb={3}>{t('offplan.singleoffplan.mainheader.tab2')}</Typography>
                                                        ...
                                                        <Box sx={{ display: "flex", alignItems: "center", gap: "20px" }}>
                                                            ...
                                                            <Box sx={{ display: "flex", alignItems: "center" }}>
                                                                ...
                                                                {el?.hidePrice ? <Typography variant='body2' whiteSpace={'noWrap'}>{t('findanagent.singleagent.contactagentforprice')}</Typography> :
                                                                    <>{ExContext?.preferredCurrency} {(ExContext?.convert(el?.currency, el?.price))?.toString()?.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} {el?.priceInterval ? ` ${t('agentdashboard.addlisting.tab6.'+el?.priceInterval)}` : el?.transactionType === "sale" ? "" : "/" + t('agentdashboard.addoffplan.tab2.month')}</>
                                                                }
                                                            </Box>
                                                        </Box>
                                                    </MuiAccordionSummary>
                                                            ...
                                                            <Grid container spacing={2.5}>
                                                                ...
                                                                <Grid item xs={6}>
                                                                    {el?.hidePrice ? <Typography variant='body2' sx={{ fontWeight: "600" }} whiteSpace={'noWrap'}>{t('findanagent.singleagent.contactagentforprice')}</Typography> :
                                                                        <Typography sx={{ fontWeight: "600" }}>{ExContext?.preferredCurrency} {(ExContext?.convert(el?.currency, el?.price))?.toString()?.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} {el?.priceInterval ? ` ${t('agentdashboard.addlisting.tab6.'+el?.priceInterval)}` : el?.transactionType === "sale" ? "" : "/" + t('agentdashboard.addoffplan.tab2.month')}</Typography>
                                                                    }
                                                                </Grid>
...

                            {
                                data?.amenities?.length > 0 &&
                                <div id="amenities">
                                    <Box id="tab2" sx={{ paddingTop: { xs: '50px', sm: '100px' } }}>
