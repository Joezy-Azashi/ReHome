import { AddCircle, ArrowCircleLeftOutlined, Close, Publish, FileDownload, RadioButtonChecked, RadioButtonUnchecked, Sell } from '@mui/icons-material'
import { TabContext, TabList, TabPanel } from '@mui/lab'
import { Box, CircularProgress, Card, TextField, InputAdornment, Pagination, Dialog, DialogContent, CardContent, Checkbox, Grid, Stack, styled, Tab, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography, IconButton, Tooltip, alpha, Backdrop, Button, ButtonGroup, Hidden } from '@mui/material'
...
const Listings = () => {
    ...
    const [openRulesPrompt, setOpenRulesPrompt] = useState(false)
    ...
    const editListing = (id, propertyType, draft) => {
        if (draft && propertyType === "development") {
            navigate('/broker/edit-off-plan', { state: { id, mode: "editMode", type: "draft" } });
        } else if (draft && propertyType !== "development") {
            navigate('/broker/edit-listing', { state: { id, mode: "editMode", type: "draft" } });
        } else if (propertyType === "development") {
            navigate('/broker/edit-off-plan', { state: { id, mode: "editMode" } });
        } else {
            navigate('/broker/edit-listing', { state: { id, mode: "editMode" } });
        }
    }
    ...
    const getListings = (wherequery = {}) => {
        ...
        Api().get("/me")
            .then((response) => {
                ...
                Api().get(`users/${response?.data?.id}/rehome-properties/count`, {
    ...
    return (
        <>
            <WrapCard elevation={0} sx={{ height: "100%" }}>
                    ...
                    <Hidden mdUp>
                        <Box sx={{ width: '100%', bgcolor: 'tertiary.main', position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 30 }}>
                            <ButtonGroup variant='text' fullWidth>
                                {(getUserType() === "agent" || getUserType() === "developer" || getUserType() === "realtor") && <Button onClick={() => navigate('/broker/newlisting')} startIcon={<AddCircle fontSize='small' sx={{ color: '#fff', }} />} fullWidth sx={{ textTransform: 'none', color: '#fff', height: '3.5rem' }}>{t('agentdashboard.home.addlisting')}</Button>}
                                {(getUserType() === "developer" || getUserType() === 'realtor') && <Button onClick={() => navigate('/broker/off-plan-listing')} variant='contained' disableElevation color='primary' startIcon={<AddCircle fontSize='small' sx={{ color: '#fff', }} />} fullWidth sx={{ textTransform: 'none', color: '#fff', height: '3.5rem' }}>{t('agentdashboard.home.addoffplan')}</Button>}
                            </ButtonGroup>
                        </Box>
                    </Hidden>

                    <Box mb={2} sx={{ display: "flex", justifyContent: "end" }}>
                        <RoundButton onClick={() => navigate('/rules-for-uploading')} text={t('agentdashboard.home.rules')} disableElevation variant={'contained'} sx={{ padding: '.5rem 1.5rem' }} />
                    </Box>
                    ...
                    <Grid container spacing={3} sx={{ height: '100%' }}>
                        <Grid item xs={12} sm={12} md={12} lg={10}>
                            <TabContext value={tabIndex}>
                                <TabList variant='scrollable' style={{
                                    ...
                                    {getUserType() === "agent" ? null : <Tab color='primary' sx={{ textTransform: 'none' }} disableRipple label={t('agentdashboard.listing.titles.tab4')} value="4" />}
                                ...
                                <TabPanel sx={{ padding: '2rem 0 0 0' }} value='1'>
                                    {/* Tables */}
                                    <TableContainer sx={{ maxHeight: maxHeight, border: `1px solid ${grey[200]}`, borderRadius: '15px', overflowX: "auto", overflowY: "hidden" }} >
                                        <Table stickyHeader >
                                            <TableHead>
                        ...
                        <Grid item xs={12} sm={12} md={12} lg={2} sx={{ height: { xs: '0', md: '70%', lg: '100%' }, display: { xs: 'block', sm: 'flex', lg: 'flex' }, flexDirection: { xs: 'row', sm: 'row', lg: 'column' }, justifyContent: { xs: 'auto', sm: 'auto', lg: 'start' } }}>
                            <Box mb={{ xs: 25, sm: 10, md: 2, lg: 3 }} mt={{ xs: -14, sm: -5, md: 2, lg: 3 }}
                                ...
                                {/* Publish Selected */}
                                <FlexBox sx={{ marginTop: { xs: "1rem", md: "-1.2rem", lg: "3.6rem" } }} onClick={() => { selectedIds.length < 1 ? enqueueSnackbar(t('agentdashboard.listing.selectitem'), { variant: 'warning' }) : setOpenRulesPrompt(true) }}>
                            ...
                            <Hidden mdDown>
                                <Box mt={5}>
                                    {/* Add Listing Button */}
                                    {(getUserType() === "developer" || getUserType() === 'realtor') &&
                                        <span onClick={() => navigate('/broker/off-plan-listing')} >
                                            <RoundButton
                                    ...
                                    {(getUserType() === "agent" || getUserType() === "developer" || getUserType() === "realtor") &&
                                        <span onClick={() => navigate('/broker/newlisting')}>
                                            <RoundButton
                                                startIcon={<AddCircle fontSize='small' sx={{ color: '#fff', }} />}
                ...
                <Dialog open={openRulesPrompt} onClose={() => setOpenRulesPrompt(false)} fullWidth maxWidth="xs">
                    <DialogContent sx={{ padding: "1.5rem 2rem" }}>
                        <Typography variant='h6' mb={2.5}>{t('agentdashboard.listing.publishdialog.title')}</Typography>
                        <Typography mb={2.5}>
                            {t('agentdashboard.listing.publishdialog.note')}
                        </Typography>

                        <Typography><NavLink to={'/rules-for-uploading'} style={{ color: "blue", textDecoration: "underline" }}>{t('agentdashboard.listing.publishdialog.clickhere')}</NavLink> {t('agentdashboard.listing.publishdialog.toread')}</Typography>
